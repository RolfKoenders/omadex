...
- `TypeMatchups.js`, `IndexSearch.js`, `PokemonDetail.js`,
  `CacheValidation.js`: pure `.pragma library` modules, no QML types, no
  side effects, no Node-only APIs. Each is directly unit-tested by loading
  it under plain Node after stripping the pragma line (see
  `tests/test_*.js`). Prefer extending these over adding logic to `Dex.qml`.
